...
function App() {
  const client = Data.map((item) => (
    <Clients img={item.img} title={item.title} name={item.Author} />
  ));

  const projectTitle = ProjectImages.map((image) => (
    <Projects img={image.img} />
  ));
  ...
  const teamInfo = teamData.map((team) => (
    <Team img={team.img} name={team.fullName} role={team.role} />
  ));
  return (
    <div>
      <h1 className=" text-3xl">WHAT OUR CLIENTS SAY</h1>
      <div className="grid grid-cols-3 grid-rows-2 gap-3 mt-8">{client}</div>
      <h1 className="m-8 text-3xl font-bold">Gallery</h1>
      <p className="mb-16">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit duis sed dapibus
        leonec.
      </p>
      <div className="grid grid-cols-3 grid-rows-3">{projectTitle}</div>
      <div className="bg-gray-200 p-8">
        <h1 className="text-3xl font-semibold mt-9">FEATURES</h1>
        <div className="grid grid-cols-4 gap-10 mt-9 ">{featuresInfo}</div>
      </div>
